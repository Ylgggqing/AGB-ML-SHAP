...
class S2FeatureWorker(QObject):
    ...
    def _extract_vi_and_reflectance(self, points_df, src):
        ...
        B, G, R, NIR = (band_arrays_reflectance.get(b, 0) for b in ["B2", "B3", "B4", "B8"])
        ...
        for i, point in points_df.iterrows():
            ...
            for name, arr in indices.items(): point_result[name] = arr[row, col]
            for name, arr in band_arrays_reflectance.items(): point_result[f'{name}_refl'] = arr[row, col]
        ...
        for i, point in points_df.iterrows():
            ...
            for band_idx in self.texture_bands:
                ...
                for w in self.window_sizes:
                    ...
                    for prop, abbr in feature_map.items(): point_result[f'B{band_idx}_W{w}_{abbr}'] = np.mean(
                        graycoprops(glcm, prop))
...
#  【修改点】: 主界面类重构为 QWidget
# ==============================================================================
class S2FeatureExtractorTab(QWidget):  # 继承自 QWidget
    def __init__(self):
        super().__init__()
        ...
        self.band_checkboxes = {f'B{b}': QCheckBox(f'B{b}') for b in [2, 3, 4, 8, 11, 12]}
        ...
        self.window_checkboxes = {size: QCheckBox(f'{size}x{size}') for size in [3, 5, 7, 9]}
    ...
    def start_extraction(self):
        ...
        self.worker = S2FeatureWorker(
            raster_path=self.raster_path_edit.text(), csv_path=self.csv_path_edit.text(),
            output_path=self.output_path_edit.text(), texture_bands=selected_bands,
            window_sizes=selected_windows, x_col='X', y_col='Y', id_col='ID'
        )
