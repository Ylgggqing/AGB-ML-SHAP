...
# 设置 Matplotlib 后端为 'Agg'，避免 GUI 线程冲突
matplotlib.use('Agg')


# ==============================================================================
#  辅助模块 - 参数对话框 (ParametersDialog) - 无需修改
# ==============================================================================
class ParametersDialog(QDialog):
    def __init__(self, model_type, initial_params, parent=None):
        ...
        if model_type == 'RandomForest':
            ...
            self.widgets['max_features'].addItems(['sqrt', 'log2', 'None']);
            self.widgets['max_features'].setCurrentText(str(self.params.get('max_features', 'sqrt')))
    ...
    def get_parameters(self):
        ...
        for name, widget in self.widgets.items():
            if isinstance(widget, QComboBox):
                updated[name] = None if widget.currentText() == 'None' else widget.currentText()
...
# ==============================================================================
#  辅助模块 - 绘图函数和工作线程 (ShapWorker) - 无需修改
# ==============================================================================
...
class ShapWorker(QObject):
    ...
    def run(self):
        try:
            ...
            model = RandomForestRegressor(**p['model_params']) if p[
                                                                      'model_type'] == 'RandomForest' else xgb.XGBRegressor(
                **p['model_params'])
            model.fit(X, y)
            ...
            if p['plot_spatial']:
                self.progress.emit(80, "正在准备空间数据...")
                coords_df = pd.read_csv(p['coords_csv'], index_col=0)
                shap_df = pd.DataFrame(shap_values, index=X.index, columns=[f'SHAP_{col}' for col in X.columns])
                combined_df = pd.concat([X, shap_df, coords_df], axis=1).dropna(subset=['X', 'Y'])
                gdf = gpd.GeoDataFrame(combined_df, geometry=gpd.points_from_xy(combined_df.X, combined_df.Y))
                self.progress.emit(90, f"正在为特征 '{p['spatial_feature']}' 生成空间分布图...");
                path = os.path.join(output_folder, f"spatial_map_{p['spatial_feature']}.png");
                plot_spatial(gdf, p['spatial_feature'], path)
...
#  【修改点】: 主界面类重构为 QWidget
# ==============================================================================
class ShapVisualizerTab(QWidget):  # 继承自 QWidget
    def __init__(self):
        super().__init__()
        # 【修改点】: 移除 setWindowTitle, setGeometry 和 setCentralWidget
        self.thread, self.worker = None, None
        self.model_params = {
            'RandomForest': {'n_estimators': 500, 'max_depth': 20, 'min_samples_split': 2, 'min_samples_leaf': 1,
                             'max_features': 'sqrt', 'random_state': 43, 'n_jobs': -1},
            'XGBoost': {'n_estimators': 500, 'max_depth': 20, 'learning_rate': 0.05, 'subsample': 0.8,
    ...
    def populate_feature_combos(self):
        ...
        if os.path.exists(path):
            try:
                ...
                self.spatial_combo.addItems(features + [f'SHAP_{f}' for f in features])
            except Exception as e:
                self.log_box.append(f"警告: 无法解析特征文件: {e}")

    def open_params_dialog(self):
        model_type = self.model_combo.currentText();
        dialog = ParametersDialog(model_type, self.model_params, self)
        if dialog.exec_() == QDialog.Accepted: self.model_params.update(
            {model_type: dialog.get_parameters()}); QMessageBox.information(self, "成功", f"{model_type} 参数已更新。")

    def _check_inputs(self):
        feature_ok = os.path.isfile(self.feature_csv_edit.text());
        output_ok = os.path.isdir(self.output_folder_edit.text())
        coords_ok = not self.spatial_check.isChecked() or os.path.isfile(self.coords_csv_edit.text())
        self.start_button.setEnabled(feature_ok and output_ok and coords_ok)
    ...
    def start_analysis(self):
        params = {
            ...
            'plot_summary': self.summary_check.isChecked(), 'plot_dependence': self.dependence_check.isChecked(),
            'dependence_feature': self.dependence_combo.currentText(), 'plot_spatial': self.spatial_check.isChecked(),
            'spatial_feature': self.spatial_combo.currentText()
        }
    ...
    def task_finished(self):
        ...
        QMessageBox.information(self, "成功", f"SHAP分析与可视化完成！\n结果已保存到: {self.output_folder_edit.text()}")
