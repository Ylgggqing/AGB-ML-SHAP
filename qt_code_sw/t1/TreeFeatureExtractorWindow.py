...
# ==============================================================================
#  后端逻辑 (Backend Logic) - TreeFeatureWorker (已完全更新)
# ==============================================================================
                ...
                try:
                    ...
                    if len(heights) > 0:
                        if self.features_to_calc.get("h_percentiles"):
                            h_percentiles = np.percentile(heights, percentile_levels)
                            result.update({f"H{p}": val for p, val in zip(percentile_levels, h_percentiles)})
                        ...
                        if self.features_to_calc.get("aih_percentiles"):
                            ...
                            if total_height > 0:
                                ...
                                aih_percentiles_vals = []
                                for p in percentile_levels:
                                    index = np.searchsorted(cumulative_percentage, p, side='right')
                                    index = min(index, len(sorted_heights) - 1)
                                    aih_percentiles_vals.append(sorted_heights[index])
                                result.update(
                                    {f"AIH{p}": val for p, val in zip(percentile_levels, aih_percentiles_vals)})
                                result["AIHiq"] = np.percentile(aih_percentiles_vals, 75) - np.percentile(
                                    aih_percentiles_vals, 25)

                        if self.features_to_calc.get("h_stats"):
                            h_mean, h_median = np.mean(heights), np.median(heights)
                            h_std, h_max, h_min = np.std(heights), np.max(heights), np.min(heights)
                            result.update({
                                "Hiq": np.percentile(heights, 75) - np.percentile(heights, 25), "Hmin": h_min,
                                "Hmax": h_max, "Hmean": h_mean, "Hmedian": h_median, "Hstd": h_std,
                                "Hvar": np.var(heights),
                                "Hcv": h_std / h_mean if h_mean != 0 else 0, "Hsq": np.sqrt(np.mean(heights ** 2)),
                                "Hcm": np.cbrt(np.mean(heights ** 3)), "Hskew": skew(heights),
                                "Hkurt": kurtosis(heights),
                    ...
                    if self.features_to_calc.get("density") and len(heights) > 1:
                        ...
                        if h_max > h_min:
                            ...
                            for k in range(len(height_bins) - 1):
                                count_in_bin = np.sum((heights >= height_bins[k]) & (heights < height_bins[k + 1]))
                                result[f"D{k}"] = count_in_bin / len(heights)
                        else:  # 如果所有点高度相同
                            for k in range(10): result[f"D{k}"] = 0.0

                    # 4. 强度相关特征
                    if len(intensities) > 0:
                        if self.features_to_calc.get("i_percentiles"):
                            i_percentiles = np.percentile(intensities, percentile_levels)
                            result.update({f"I{p}": val for p, val in zip(percentile_levels, i_percentiles)})
                        ...
                        if self.features_to_calc.get("aii_percentiles"):
                            ...
                            if total_intensity > 0:
                                ...
                                aii_percentiles_vals = []
                                for p in percentile_levels:
                                    index = np.searchsorted(cumulative_percentage, p, side='right')
                                    index = min(index, len(sorted_intensities) - 1)
                                    aii_percentiles_vals.append(sorted_intensities[index])
                                result.update(
                                    {f"AII{p}": val for p, val in zip(percentile_levels, aii_percentiles_vals)})
                                result["AIIiq"] = np.percentile(aii_percentiles_vals, 75) - np.percentile(
                                    aii_percentiles_vals, 25)

                        if self.features_to_calc.get("i_stats"):
                            i_mean, i_median = np.mean(intensities), np.median(intensities)
                            i_std, i_max, i_min = np.std(intensities), np.max(intensities), np.min(intensities)
                            result.update({
                                ...
                                "Ivar": np.var(intensities),
                                "Icv": i_std / i_mean if i_mean != 0 else 0, "Iskew": skew(intensities),
...
# ==============================================================================
#  前端界面 (Frontend UI) - TreeFeatureExtractorWindow (已更新)
# ==============================================================================
class TreeFeatureExtractorWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("单木LiDAR特征提取工具")
        self.setGeometry(100, 100, 650, 600)  # 调整窗口大小以适应更多选项
    ...
    def start_extraction(self):
        ...
        features_to_calculate = {
            name: checkbox.isChecked() for name, checkbox in self.feature_checkboxes.items()
        }
        ...
        # self.start_button.clicked.connect(self.stop_extraction)
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.folder_path_edit.parent().setEnabled(enabled)  # 禁用路径设置框
        self.output_path_edit.parent().setEnabled(enabled)
...
if __name__ == '__main__':
    ...
    window = TreeFeatureExtractorWindow()
    window.show()
    sys.exit(app.exec_())