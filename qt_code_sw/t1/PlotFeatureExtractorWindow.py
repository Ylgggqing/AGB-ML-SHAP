...
# ==============================================================================
#  后端逻辑 (Backend Logic) - PlotFeatureWorker (已完全更新)
# ==============================================================================
                ...
                try:
                    ...
                    if len(heights) > 0:
                        ...
                        if self.features_to_calc.get("aih_percentiles"):
                            ...
                            if total_height > 0:
                                ...
                                for p in percentile_levels:
                                    ...
                                    aih_vals.append(sorted_heights[index])
                        ...
                        if self.features_to_calc.get("h_stats"):
                            ...
                            h_max, h_min = np.max(heights), np.min(heights)
                            result.update({
                    ...
                    if len(intensities) > 0:
                        if self.features_to_calc.get("i_percentiles"):
                            i_percentiles_vals = np.percentile(intensities, percentile_levels)
                            result.update({f"I{p}": val for p, val in zip(percentile_levels, i_percentiles_vals)})
                        ...
                        if self.features_to_calc.get("aii_percentiles"):
                            ...
                            if total_intensity > 0:
                                ...
                                for p in percentile_levels:
                                    ...
                                    aii_vals.append(sorted_intensities[index])
                        ...
                        if self.features_to_calc.get("i_stats"):
                            ...
                            result.update({
                                "Iiq": np.percentile(intensities, 75) - np.percentile(intensities, 25),
                                ...
                                "Isq": np.sqrt(np.mean(intensities ** 2)), "Icm": np.cbrt(np.mean(intensities ** 3)),
                                "Iskew": skew(intensities), "Ikurt": kurtosis(intensities),
                                "Imad": np.mean(np.abs(intensities - i_mean))
                            })

                    # --- 体积指数 ---
                    if self.features_to_calc.get("volumetric_indices"):
                        vol_indices = self._calculate_volumetric_indices(df)
                        result.update(vol_indices)
...
# ==============================================================================
#  前端界面 (Frontend UI) - PlotFeatureExtractorWindow (已更新)
# ==============================================================================
class PlotFeatureExtractorWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("样地尺度LiDAR特征提取工具")
        self.setGeometry(100, 100, 650, 700)  # 调整窗口大小
        ...
        self.feature_checkboxes = {
            "h_stats": QCheckBox("高度基本统计 (Hmean, Hstd 等)"),
    ...
    def start_extraction(self):
        ...
        volumetric_params = {
            "z_threshold": self.z_thresh_spin.value(),
            "voxel_size": self.voxel_size_spin.value(),
            "vertical_segment_size": self.v_seg_size_spin.value(),
            "k_factor": self.k_factor_spin.value()
        }
        ...
        self.worker = PlotFeatureWorker(
            ...
            volumetric_params=volumetric_params
        )
...
if __name__ == '__main__':
    ...
    window = PlotFeatureExtractorWindow()
    window.show()
    sys.exit(app.exec_())