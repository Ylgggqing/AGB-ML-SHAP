...
# ==============================================================================
#  后端逻辑 (Backend Logic) - AggregationWorker
#  注意：由于并行处理的复杂性，辅助函数放在类外部以便pickle
# ==============================================================================
...
class AggregationWorker(QObject):
    ...
    finished = pyqtSignal(str, str)  # output_shp, output_csv
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=self.params['num_workers']) as executor:
                results = list(executor.map(
                    _process_single_plot,
                    plots_data,
                    [tree_groups] * len(plots_data),
                    [agb_map_df] * len(plots_data),
                    [tree_hulls_dict] * len(plots_data),
            ...
            output_shp_path = self.params['output_shp']
            output_csv_path = self.params['output_csv']

            final_gdf.drop(columns=['TreeCount']).to_file(output_shp_path, driver='ESRI Shapefile', encoding='utf-8')
            pd.DataFrame(final_gdf.drop(columns='geometry')).to_csv(output_csv_path, index=False, encoding='utf-8-sig')

            self.finished.emit(output_shp_path, output_csv_path)
        except Exception as e:
            self.error.emit(f"发生严重错误: {e}")


# ==============================================================================
#  前端界面 (Frontend UI) - AggregationWindow
# ==============================================================================
class AggregationWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("样地AGB体积比例聚合法工具")
        ...
        self.toggle_plot_widgets(False)  # Initially disable
    ...
    def select_points_folder(self):
        path = QFileDialog.getExistingDirectory(self, "选择单木点云文件夹")
        if path: self.points_folder_edit.setText(path)
        self._check_inputs()

    def select_grid_shp(self):
        path, _ = QFileDialog.getOpenFileName(self, "选择样地格网Shapefile", "", "Shapefile (*.shp)")
        if path: self.grid_shp_edit.setText(path)
        self._check_inputs()

    def select_agb_csv(self):
        path, _ = QFileDialog.getOpenFileName(self, "选择单木AGB映射CSV", "", "CSV 文件 (*.csv)")
        if path: self.agb_csv_edit.setText(path)
        self._check_inputs()

    def select_output_shp(self):
        path, _ = QFileDialog.getSaveFileName(self, "选择输出Shapefile路径", "", "Shapefile (*.shp)")
        if path: self.output_shp_edit.setText(path)
        self._check_inputs()

    def select_output_csv(self):
        path, _ = QFileDialog.getSaveFileName(self, "选择输出CSV路径", "", "CSV 文件 (*.csv)")
        if path: self.output_csv_edit.setText(path)
        self._check_inputs()

    def select_plot_path(self):
        path, _ = QFileDialog.getSaveFileName(self, "选择概览图保存路径", "", "PNG 文件 (*.png)")
        if path: self.plot_path_edit.setText(path)

    def toggle_plot_widgets(self, checked):
        self.plot_path_edit.setEnabled(checked)
        self.plot_path_button.setEnabled(checked)

    def _check_inputs(self):
        all_ok = all([
            os.path.isdir(self.points_folder_edit.text()),
            os.path.exists(self.grid_shp_edit.text()),
            os.path.exists(self.agb_csv_edit.text()),
            bool(self.output_shp_edit.text()),
    ...
    def start_aggregation(self):
        params = {
            'points_folder': self.points_folder_edit.text(),
            'grid_shp': self.grid_shp_edit.text(),
            'agb_csv': self.agb_csv_edit.text(),
            'output_shp': self.output_shp_edit.text(),
        ...
        if params['generate_plot'] and not params['plot_path']:
            QMessageBox.warning(self, "警告", "请为概览图指定一个保存路径。")
            return
...
if __name__ == '__main__':
    # PyInstaller/multiprocessing on Windows requires this guard
